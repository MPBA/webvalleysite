...
def do_final_submission(user_profile):
    ...
    c.drawImage(os.path.join(data_path, profile_picture), pagesize[0] - 200, pagesize[1] - 200, 150, 150,
    ...
    for form_filename in os.listdir(sf_dir):
        if os.path.splitext(form_filename)[1].lower() == '.pdf':
            merger.append(PdfFileReader(open(os.path.join(sf_dir, form_filename), 'rb'), strict=False))
...
def _send_mail_from_template(template, context, recipients):
    ...
    send_mail(APPLICATIONPROCESS_EMAIL_SUBJECT, mail_body, WEBVALLEY_EMAIL_ADDRESS, ['luca.coviello@studenti.unitn.it'],
              fail_silently=False)
